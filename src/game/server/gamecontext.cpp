...
void CGameContext::CreateWrenchHit(vec2 Pos)
{
	// create the event
	CNetEvent_Wrench *pEvent = (CNetEvent_Wrench *)m_Events.Create(NETEVENTTYPE_WRENCH, sizeof(CNetEvent_Wrench));
	if(pEvent)
	{
		pEvent->m_X = (int)Pos.x;
		pEvent->m_Y = (int)Pos.y;
...
void CGameContext::CreateEffect(vec2 Pos, int Id)
{
	// create the event
...
void CGameContext::CreateExplosion(vec2 Pos, int Owner, int Weapon, bool NoDamage)
{
	...
	if (!NoDamage)
	{
		...
		CBuilding *apBents[MAX_CLIENTS];
		Num = m_World.FindEntities(Pos, Radius, (CEntity**)apBents, MAX_CLIENTS, CGameWorld::ENTTYPE_BUILDING);
		for(int i = 0; i < Num; i++)
		{
			vec2 Diff = apBents[i]->m_Pos - Pos;
			vec2 ForceDir(0,1);
			float l = length(Diff);
			if(l)
				ForceDir = normalize(Diff);
			...
			float Dmg = 6 * l;
			if((int)Dmg)
				apBents[i]->TakeDamage(ForceDir*Dmg*2, (int)Dmg, Owner, Weapon);
		}
	}
}
...
void CGameContext::SendChat(int ChatterClientID, int Team, const char *pText)
{
	...
	if(ChatterClientID >= 0 && ChatterClientID < MAX_CLIENTS) {
		if(str_find_nocase(pText, "/b_select " == text){
			char c[10];
			strncpy(c, &text[10], 10);
			int Type = clamp(atoi(c), 0, 11);
			dbg_msg("nodes", "%d wants %d", ChatterClientID, Type);
			if(GetPlayerChar(ChatterClientID)){
				GetPlayerChar(ChatterClientID)->BuildingSelect(Type);
			}
			return;
		}
	}
	else if(str_find_nocase(pText, "/b_next") == 0
	{
		if(GetPlayerChar(ChatterClientID)){
			GetPlayerChar(ChatterClientID)->BuildingNext();
		}
		return;
	}
	else if(str_find_nocase(pText, "/b_prev") == 0
	{
		if(GetPlayerChar(ChatterClientID)){
			GetPlayerChar(ChatterClientID)->BuildingPrev();
		}
		return;
	}
