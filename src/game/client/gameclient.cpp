...
#include "components/buildmenu.h"
...
static CBuildmenu gs_Buildmenu;
...
//	m_All.Add(&gs_Scoreboard);
	...
	Console()->Register("set_techlevel", "ii", CFGFLAG_SERVER, 0, 0, "Set techlevel");
...
void CGameClient::OnInit()
{
	...
	// Set to disable change in CONFIG. [didn't do with console.]
	...
	for(int i = 0; i < g_pData->m_NumImages; i++)
	{
		...
		g_GameClient.m_pMenus->RenderLoading(/*g_pData->m_aImages[i].m_pFilename*/);
	}
...
void CGameClient::OnRender()
{
	...
	for(int i = 0; i < m_All.m_Num; i++)
	{
		...
		//if (m_All.m_paComponents[i] == &
		//NODESTODO:Found out the render2
	}
...
void CGameClient::OnMessage(int MsgId, CUnpacker *pUnpacker)
{
	...
	else if(MsgId == NETMSGTYPE_SV_TECHLEVELUPDATE) {
		CNetMsg_Sv_TechlevelUpdate *pMsg = (CNetMsg_Sv_TechlevelUpdate *)pRawMsg;
		m_Tech[0] = pMsg->m_T1;
		m_Tech[1] = pMsg->m_T2;
		Collision()->SetTech(0, m_Tech[0]);
		Collision()->SetTech(1, m_Tech[1]);
	}
}
...
void CGameClient::ProcessEvents()
{
	...
	for(int Index = 0; Index < Num; Index++)
	{
		...
		else if(Item.m_Type == NETEVENTTYPE_WRENCH)
		{
			CNetEvent_Wrench *ev = (CNetEvent_Wrench *)pData;
			g_GameClient.m_pEffects->WrenchHit(vec2(ev->m_X, ev->m_Y));
		}
		else if(Item.m_Type == NETEVENTTYPE_EFFECT)
		{
			CNetEvent_Effect *ev = (CNetEvent_Effect *)pData;
			switch (ev->m_EffectID)
			{
				case 0: g_GameClient.m_pEffects->Fallout(vec2(ev->m_X, ev->m_Y)); break;
				case 1: g_GameClient.m_pEffects->Healing(vec2(ev->m_X, ev->m_Y)); break;
				case 2: g_GameClient.m_pEffects->Emp(vec2(ev->m_X, ev->m_Y)); break;
				case 3: g_GameClient.m_pEffects->IceExpl(vec2(ev->m_X, ev->m_Y)); break;
			}
		}
		...
		else if(Item.m_Type == NETEVENTTYPE_POSITIONUPDATE)
		{
			CNetEvent_PositionUpdate *ev = (CNetEvent_PositionUpdate *)pData;
			//NODESTODO:Find out this function below
			m_LocalCharacterPos = vec2(ev->m_X, ev->m_Y);
		}
	}
}
	...
	// go trough all the items in the snapshot and gather the info we want
	{
		...
		for(int i = 0; i < Num; i++)
		{
			...
			else if(Item.m_Type == NETOBJTYPE_GAMEDATA)
			{
				...
				m_Snap.m_paBuildings[Item.m_ID%2] = (const CNetObj_Building *)pData;
		}	
	}
...
void CGameClient::SendInfo(bool Start)
{
	if(Start)
	{
		...
		Msg.m_ColorBody = 0xDEADBEEF;
		Msg.m_ColorFeet = 0xBADC0FFF;
		Client()->SendPackMsg(&Msg, MSGFLAG_VITAL);
	}
	else
	{
