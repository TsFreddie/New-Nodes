...
void CEffects::FireTrail(vec2 Pos, vec2 Vel)
{
	if(!m_Add50hz)
		return;
	...
	p.m_Friction = 0.7f;
	p.m_Gravity = frandom()*-500.0f;
	p.m_Color = vec4(0.8f+frandom()*0.2f, 0.0f, 0.0f, 1.0f);
	m_pClient->m_pParticles->Add(CParticles::GROUP_PROJECTILE_TRAIL, &p);
}
...
{
	vec3 FalloutColor(0.0f,1.0f,0.0f);
	...
	for(int i = 0; i < 256; i++)
	{
		...
		p.m_LifeSpan = 5.0f + frandom()*20.0f;
	...
	p.m_Friction = 0.0f;
	p.m_Fadeout = true;
	p.m_Color = vec4(FalloutColor.r, FalloutColor.g, FalloutColor.b, 0.15f);
	m_pClient->m_pParticles->Add(CParticles::GROUP_GENERAL, &p);
}
...
void CEffects::WrenchHit(vec2 Pos)
{
	// add the explosion
	CParticle p;
	p.SetDefault();
	...
	p.m_Rot = frandom()*pi*2;
	m_pClient->m_pParticles->Add(CParticles::GROUP_EXPLOSIONS, &p);
	m_pClient->m_pSounds->PlayAt(CSounds::CHN_WORLD, SOUND_BUILD, 1.0f, Pos);
}
...
void CEffects::IceExpl(vec2 pos) {
	...
	p.m_Friction = 0.0f;
	p.m_Fadeout = true;
	//p.color = vec4(0.9f, 0.9f, 1.0f, 0.5f);// mix(vec4(0.75f,0.75f,0.75f,1.0f), vec4(0.5f,0.5f,0.5f,1.0f), frandom());
	m_pClient->m_pParticles->Add(CParticles::GROUP_GENERAL, &p);
}
