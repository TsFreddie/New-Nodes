	...
	)
{
	...
	bool IsIll = false;
	if (pInfo.m_LatencyFlux & 2){
		IsIll = true;
		m_pClient->m_aClients[pInfo.m_ClientID].m_Emoticon &= 0x7FFFFFFF;
	}
	...
	if (IsIll){
		m_aRenderInfo[i].m_ColorBody = vec4(0,1,0,1);
	}
	...
	if (IsIll && NewTick % 100 == 0){
		m_pClient->m_Effects->Fallout2(Position);
	}
