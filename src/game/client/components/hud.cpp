...
void CHud::RenderScoreHud()
{
	...
	if(!(m_pClient->m_Snap.m_pGameInfoObj->m_GameStateFlags&GAMESTATEFLAG_GAMEOVER))
	{
		...
		// NODES: Render Quick Help
		Graphics()->BlendNormal();
		Graphics()->TextureSet(-1);

		//dbg_msg("nodes", ">enter tick %lld\n<clienttick %lld", gameclient.enter_time, time_get());
		if(m_pClient->m_EnterTime+25000000 >= time_get() && g_Config.m_ClQuickHelp ) {
			Graphics()->QuadsBegin();
			Graphics()->SetColor(0,0,0,0.25f);
			RenderTools()->DrawRoundRect(Whole-160, 300-40-15-50, 170, 48, 5.0f);
			Graphics()->QuadsEnd();
			char aBuf[256];
			str_format(aBuf, sizeof(aBuf), Localize("Quick help:\nPress %s to open the build menu\nPress %s for deconstruction\nDisable this help in the settings"), m_pClient->m_pBinds->GetKey("+buildmenu"), m_pClient->m_pBinds->GetKey("deconstruct"));
			TextRender()->Text(0, Whole-155, 300-40-15-47, 9, aBuf, 160);
		}
		
		//NODES: Preload
		enum {CRATE_HEALTH = 1, CRATE_ARMOR, CRATE_CONSTRUCTION, CRATE_BASEBALL, CRATE_FREEZE, CRATE_EMP, CRATE_DUCK, CRATE_BOOM}; //TODO: fix
		...
		if(GameFlags&GAMEFLAG_TEAMS && m_pClient->m_Snap.m_pGameDataObj)
		{
			...
			for(int t = 0; t < 2; t++)
			{
				...
				Graphics()->SetColor(0.0f, 1.0f, 1.0f, 0.25f);
				/*
				if(t == 0)
				...
				*/
				...
				if(t == 1)	
				{
					if(m_pClient->m_Snap.m_pLocalCharacter->m_Crate == CRATE_CONSTRUCTION)
					{
						str_format(aBuf, sizeof(aBuf), "%s", Localize("FREE"));
						float TextWidth = TextRender()->TextWidth(0, 14, aBuf, -1);
						TextRender()->Text(0, Whole-ScoreWidthMax+(ScoreWidthMax-TextWidth)/2-Split, StartY+t*20, 14.0f, aBuf, -1);
					}
					else
			...
			const char *name = Localize("Tech Level");
			float w = TextRender()->TextWidth(0, 10, name, -1);
			TextRender()->Text(0, Whole-ScoreWidthMax-ImageSize-2*Split-w-5,StartY-8.0f + 2, 10, name, -1); 
			name = Localize("Build Points");
			w = TextRender()->TextWidth(0, 10, name, -1);
			TextRender()->Text(0, Whole-ScoreWidthMax-ImageSize-2*Split-w-5,StartY + 20 + 2, 10, name, -1); 
		}
		else
		{
			int Local = -1;
...
void CHud::RenderCursor()
{
	...
	// render cursor
	if (m_pClient->m_Snap.m_pLocalCharacter->m_Crate == CRATE_BASEBALL & m_pClient->m_Snap.m_pLocalCharacter->m_Weapon == WEAPON_HAMMER)
	{
		RenderTools()->SelectSprite(SPRITE_WEAPON_BASEBALL_CURSOR);
	}
	else
	{
...
void CHud::RenderHealthAndAmmo(const CNetObj_Character *pCharacter)
{
	...
	for (i = 0; i < min(pCharacter->m_AmmoCount, 10); i++)
		Array[i] = IGraphics::CQuadItem(x+i*12,y+20,10,10);
	...
	// Render Rage
	int Rage = m_pClient->m_Snap.m_pLocalCharacter->m_Rage;
	Rage = 90;
	...
	RenderTools()->DrawCirclePart(5+18.0f, 5+18.0f, 16.0f, 100, Rage/100.0f);
	Graphics()->QuadsEnd();
	
	Graphics()->TextureSet(g_pData->m_aImages[IMAGE_GUI].m_Id);
	Graphics()->QuadsBegin();
	RenderTools()->SelectSprite(SPRITE_GUI_RAGE);
	Array[0] = IGraphics::CQuadItem(5, 5, 36, 36);
	Graphics()->QuadsDrawTL(Array, 1);
	
	int Spr = -1;
	if (Rage >= 90)
		Spr = SPRITE_GUI_RAGE_RAMPAGE;
	else if (Rage >= 70)
		Spr = SPRITE_GUI_RAGE_BOMB;
	else if (Rage >= 40)
		Spr = SPRITE_GUI_RAGE_HOT;
	
	if (Spr >= 0)
	{
		RenderTools()->SelectSprite(Spr);
		Array[0] = IGraphics::CQuadItem(11, 11, 24, 24);
		Graphics()->QuadsDrawTL(Array, 1);
	}
	...
	// render health
	RenderTools()->SelectSprite((m_pClient->m_Snap.m_pLocalCharacter->m_Health < 1 ? SPRITE_GUI_EMPTY_START : SPRITE_GUI_HEALTH_START));
	Array[0] = IGraphics::CQuadItem(x,y,12,12);
	Graphics()->QuadsDrawTL(Array, 1);
	
	RenderTools()->SelectSprite(SPRITE_GUI_HEALTH);
	for(; h < min(pCharacter->m_Health, 9); h++)
	{
		Array[0] = IGraphics::CQuadItem(x+h*12,y,12,12);
		Graphics()->QuadsDrawTL(Array, 1);
	}
	
	RenderTools()->SelectSprite(SPRITE_GUI_EMPTY);
	for(; h < 9; h++)
	{
		Array[0] = IGraphics::CQuadItem(x+h*12,y,12,12);
		Graphics()->QuadsDrawTL(Array, 1);
	...
	RenderTools()->SelectSprite((m_pClient->m_Snap.m_pLocalCharacter->m_Health < 10 ? SPRITE_GUI_EMPTY_END : SPRITE_GUI_HEALTH_END));
	Array[0] = IGraphics::CQuadItem(x+9*12,y,12,12);
	Graphics()->QuadsDrawTL(Array, 1);

	// render armor meter
	h = 1;
	RenderTools()->SelectSprite((m_pClient->m_Snap.m_pLocalCharacter->m_Armor < 1 ? SPRITE_GUI_EMPTY_START : SPRITE_GUI_ARMOR_START));
	Array[0] = IGraphics::CQuadItem(x,y+10,12,12);
	Graphics()->QuadsDrawTL(Array, 1);
	
	RenderTools()->SelectSprite(SPRITE_GUI_ARMOR);
	for(; h < min(pCharacter->m_Armor, 9); h++)
	{
		Array[0] = IGraphics::CQuadItem(x+h*12,y+10,12,12);
		Graphics()->QuadsDrawTL(Array, 1);
	}
	...
	RenderTools()->SelectSprite((m_pClient->m_Snap.m_pLocalCharacter->m_Armor < 10 ? SPRITE_GUI_EMPTY_END : SPRITE_GUI_ARMOR_END));
	Array[0] = IGraphics::CQuadItem(x+9*12,y+10,12,12);
	Graphics()->QuadsDrawTL(Array, 1);
