...
void CParticles::RenderGroup(int Group)
{
	...
	while(i != -1)
	{
		if (m_aParticles[i].m_Set != -1){
			Graphics()->QuadsEnd();
			Graphics()->TextureSet(g_pData->m_aImages[m_aParticles[i].m_Set].m_Id);
			CustomSet = true;
			Graphics()->QuadsBegin();
		}
		else if (CustomSet){
		...
		Graphics()->SetColor(
			...
			m_aParticles[i].m_Color.a*(m_aParticles[i].m_Fadeout ? (1-a) :1) ); // pow(a, 0.75f) *
