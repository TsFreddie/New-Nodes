...
public:
	...
	void FireTrail(vec2 Pos, vec2 Vel);
	void SkidTrail(vec2 Pos, vec2 Vel);
	void Explosion(vec2 Pos);
	void HammerHit(vec2 Pos);
	void WrenchHit(vec2 Pos);
	void Fallout(vec2 Pos);
	void Fallout2(vec2 Pos);
	void Healing(vec2 Pos);
	...
	void Smoke(vec2 Pos, vec2 Dir);
	void Emp(vec2 Pos);
	void IceExpl(vec2 Pos);
	void Update();
};
#endif
