...
void CRenderTools::DrawCirclePart(float x, float y, float r, int Segs, float Percentage)
{
	IGraphics::CFreeformItem Array[1];
	
	for (int i = 0; i < Percentage*Segs; i++)
	{
		float Alpha = i*2*pi/Segs;
		float x1 = x+r*sinf(Alpha);
		float y1 = y-r*cosf(Alpha);
		
		Alpha = (i+1)*2*pi/Segs;
		
		float x2 = x+r*sinf(Alpha);
		float y2 = y-r*cosf(Alpha);
		
		Array[0] = IGraphics::CFreeformItem(
			x, y,
			x, y,
			x1, y1,
			x2, y2);
		Graphics()->QuadsDrawFreeform(Array, 1);
	}
}
